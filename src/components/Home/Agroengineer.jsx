...
export default function Nogaproduct() {
  ...
  const GetNogaProduct = async () => {
    try {
      ...
      const response = await makeRequest("POST", "/Product/GetProductBycategory", { Id: 4 }, { headers: { Authorization: `Bearer ${storedToken}` } });
      ...
      const mappedProducts = response.dataset.$values.map((item) => {
        ...
        return {
          id: item.proD_ID,
          category: item.categorY_ID === "4" ? "Agro-engineer" : "Unknown",
  ...
  return (
    <div className="container mx-auto p-4 font-poppins">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-xl md:text-2xl font-bold">Agroengineer Product</h1>
        <Link href="/Category?category_id=4" className="flex items-center text-green-700 hover:text-green-900">
