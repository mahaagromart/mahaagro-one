...
export default function Nogaproduct() {
  ...
  const GetNogaProduct = async () => {
    try {
      ...
      const response = await makeRequest("POST", "/Product/GetProductBycategory", { Id: 12 }, { headers: { Authorization: `Bearer ${storedToken}` } });
      ...
      const mappedProducts = response.dataset.$values.map((item) => {
        ...
        return {
          id: item.proD_ID,
          category: item.categorY_ID === "12" ? "Gardening" : "Unknown",
